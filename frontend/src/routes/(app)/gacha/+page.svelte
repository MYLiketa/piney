...
<div class="container max-w-5xl mx-auto py-12 flex flex-col items-center justify-center min-h-[80vh] space-y-12">
    ...
    <div class="relative w-full h-[450px] sm:h-[550px] flex items-center justify-center perspective-1000">
        ...
        {#if phase === 'ready'}
             ...
             {:else}
                 <div class="relative w-[48vw] h-[72vw] sm:w-64 sm:h-96 shrink-0"
                     ...
                     {#each Array(displayCardCount) as _, i}
                         <div class="absolute inset-0 rounded-xl overflow-hidden shadow-2xl" 
                              style="transform: rotate({i * 9 - (displayCardCount - 1) * 4.5}deg) translate({i * 12 - (displayCardCount - 1) * 6}px, -{i * 4}px); z-index: {i}; box-shadow: 0 10px 30px rgba(0,0,0,0.5); isolation: isolate; -webkit-mask-image: -webkit-radial-gradient(white, black);">
                             {@html CardBack}
                         </div>
                     {/each}
                 </div>
        ...
        {:else if phase === 'shuffling'}
             <!-- Shuffling Animation (动态数量) -->
              {#each Array(displayCardCount) as _, i}
                 <div class="absolute w-[28vw] h-[42vw] sm:w-64 sm:h-96 shrink-0 rounded-xl overflow-hidden shadow-2xl animate-shuffle-{i}"
                      style="z-index: {10 - i}; isolation: isolate; -webkit-mask-image: -webkit-radial-gradient(white, black);">
                     {@html CardBack}
                 </div>
              {/each}
        ...
        {:else if phase === 'spreading'}
             ...
             <div class="flex gap-4 sm:gap-8 items-center justify-center w-full">
                 {#each cards as card, i}
                     ...
                     <div class="relative w-[28vw] h-[42vw] sm:w-64 sm:h-96 shrink-0 rounded-xl overflow-hidden shadow-2xl animate-spread-{i}" style="isolation: isolate; -webkit-mask-image: -webkit-radial-gradient(white, black);">
                         {@html CardBack}
                     </div>
                 {/each}
             </div>
        ...
        {:else}
              ...
              <div class="flex gap-4 sm:gap-8 items-center justify-center w-full">
                  {#each cards as card, i}
                      ...
                      <button 
                           class="relative w-[28vw] h-[42vw] sm:w-64 sm:h-96 shrink-0 perspective-1000 group outline-none focus:outline-none bg-transparent border-none p-0 transition-transform duration-300 {phase === 'picking' ? 'hover:-translate-y-6 hover:shadow-purple-500/30' : ''}"
                      ...
                      >
                          ...
                          <div class="w-full h-full relative preserve-3d transition-all duration-700"
                              ...
                              <div class="absolute inset-0 backface-hidden rounded-xl overflow-hidden shadow-2xl" style="isolation: isolate; -webkit-mask-image: -webkit-radial-gradient(white, black);">
                                  {@html CardBack}
                              </div>

                              <!-- Back (Content) -->
                              <div class="absolute inset-0 backface-hidden rotate-y-180 rounded-xl overflow-hidden shadow-xl bg-card flex flex-col items-center bg-zinc-900">
                                  <!-- Image -->
                                  <div class="w-full h-full relative group">
                                      <img src={resolveUrl(card.avatar)} alt={card.name} class="w-full h-full object-cover" />
                                      <div class="absolute inset-0 bg-gradient-to-t from-black/90 via-black/20 to-transparent"></div>
                                      
                                      <div class="absolute bottom-0 left-0 right-0 p-3 sm:p-6 text-center transform translate-y-2 group-hover:translate-y-0 transition-transform">
                                          <h3 class="text-white text-sm sm:text-2xl font-bold drop-shadow-lg mb-1">{card.name}</h3>
                                          {#if card.description}
                                              <p class="text-white/70 text-[10px] sm:text-xs line-clamp-2 opacity-0 group-hover:opacity-100 transition-opacity delay-100">
                                                  {card.description}
                                              </p>
                                          {/if}
                                      </div>
                          ...
                          {#if phase === 'revealed' && selectedIndex === i}
                               <div class="absolute -inset-1 sm:-inset-4 rounded-[16px] sm:rounded-[20px] ring-2 ring-primary/50 ring-offset-2 ring-offset-background animate-pulse pointer-events-none z-20"></div>
                          {/if}
                      </button>
                  {/each}
              </div>
