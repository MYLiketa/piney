<script lang="ts">
    ...
    let cardToDelete: string | null = $state(null);
    let isBatchDeleteArgs = $state(false);
    let moveDialogOpen = $state(false);
    let batchTagDialogOpen = $state(false);
    let batchAddTagInput = $state("");
    let batchRemoveTagInput = $state("");
    ...
    function getCurrentListUrl() {
        const url = new URL(window.location.origin + "/characters");
        if (searchQuery) {
            url.searchParams.set("search", searchQuery);
        }
        if (selectedCategoryId === null) {
            url.searchParams.set("category_id", "null");
    ...
    async function handleBatchExport() {
        ...
        try {
            ...
            if (selectedCardIds.size === 1) {
                ...
                await downloadFile({ filename: `${name}.${ext}`, content: blob });
            } else {
    ...
    async function confirmBatchTagUpdate() {
        if (selectedCardIds.size === 0) return;
        const addTags = batchAddTagInput.split(',').map(t => t.trim()).filter(t => t.length > 0);
        const removeTags = batchRemoveTagInput.split(',').map(t => t.trim()).filter(t => t.length > 0);

        if (addTags.length === 0 && removeTags.length === 0) {
            toast.error("请至少输入一个要添加或移除的标签");
            return;
        }
        ...
        if (failCount === 0) {
            toast.success(`成功更新 ${successCount} 个角色的标签`);
        } else {
    ...
    function softDeleteCard(id: string) {
        if (isSelectionMode && selectedCardIds.has(id) && selectedCardIds.size > 1) {
    ...
    function applyUrlParams() {
        ...
        selectedTags = tagsParam ? tagsParam.split(',').filter(t => t.trim()) : [];
    ...
    onMount(async () => {
        ...
        applyUrlParams();
        const savedSort = localStorage.getItem("characters_sort");
        const savedOrder = localStorage.getItem("characters_order");
        if (savedSort) currentSort = savedSort;
        if (savedOrder) currentOrder = savedOrder;
        await refreshData();
    });

    // ============ 响应式 ============
    let paginatedCards = $derived.by(() => cards);
    ...
    $effect(() => {
        ...
        const tagsChanged = selectedTags.join(',') !== prevTags.join(',');

        if (searchChanged || categoryChanged || sortChanged || orderChanged || pageChanged || tagsChanged) {
            if ((searchChanged || categoryChanged || sortChanged || orderChanged || tagsChanged) && !pageChanged) {
                currentPage = 1;
            }
    ...
    // 保存排序偏好
    $effect(() => {
        localStorage.setItem("characters_sort", currentSort);
        localStorage.setItem("characters_order", currentOrder);
    });
    ...
    $effect(() => {
        ...
        // 恢复分类
        const catParam = urlParams.get('category_id');
        if (catParam === 'null' || catParam === null) {
            if (selectedCategoryId !== null) selectedCategoryId = null;
        } else if (catParam && selectedCategoryId !== catParam) {
            selectedCategoryId = catParam;
        }
        
        // 恢复标签
        const tagsParam = urlParams.get('tags');
        const tagsArray = tagsParam ? tagsParam.split(',').filter(t => t.trim()) : [];
        if (tagsArray.join(',') !== selectedTags.join(',')) {
    ...
    let selectRect = $state({ x: 0, y: 0, width: 0, height: 0 });
    let dragStartX = 0;
    let dragStartY = 0;
    let maybeDragSelect = false;
    let hasMovedOverThreshold = false;

    function getContainerCoords(e: MouseEvent) {
        if (!galleryContainer) return { x: 0, y: 0 };
        const rect = galleryContainer.getBoundingClientRect();
        return {
            x: e.clientX - rect.left + galleryContainer.scrollLeft,
            y: e.clientY - rect.top + galleryContainer.scrollTop,
        };
    }
    ...
    function handleMouseDown(e: MouseEvent) {
        ...
        dragStartX = coords.x;
        dragStartY = coords.y;
        selectRect = { x: dragStartX, y: dragStartY, width: 0, height: 0 };
        maybeDragSelect = true;
        hasMovedOverThreshold = false;
    }

    function handleMouseMove(e: MouseEvent) {
        if (!maybeDragSelect || !galleryContainer) return;
        e.preventDefault();
        const coords = getContainerCoords(e);
        ...
        const x = Math.min(coords.x, dragStartX);
        const y = Math.min(coords.y, dragStartY);
        const width = Math.abs(coords.x - dragStartX);
        const height = Math.abs(coords.y - dragStartY);
        selectRect = { x, y, width, height };

        const cards = galleryContainer.querySelectorAll('[role="button"]');
        const containerRect = galleryContainer.getBoundingClientRect();
        cards.forEach((card) => {
            const cardRect = card.getBoundingClientRect();
            const cardLeft = cardRect.left - containerRect.left + galleryContainer.scrollLeft;
            const cardTop = cardRect.top - containerRect.top + galleryContainer.scrollTop;
            const cardRight = cardLeft + cardRect.width;
            const cardBottom = cardTop + cardRect.height;
            const rectRight = selectRect.x + selectRect.width;
            const rectBottom = selectRect.y + selectRect.height;
            const intersect = !(
                cardRight < selectRect.x ||
                cardLeft > rectRight ||
                cardBottom < selectRect.y ||
                cardTop > rectBottom
            );
    ...
    function handleMouseUp(e: MouseEvent) {
        ...
        maybeDragSelect = false;
        selectRect = { x: 0, y: 0, width: 0, height: 0 };
    }
</script>
...
<div class="container py-6 space-y-6 max-w-7xl mx-auto">
    ...
    {:else if viewMode === "gallery"}
        ...
        <div
            ...
            class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-5 relative select-none"
            role="application"
        ...
        >
            <!-- 拖拽框选的矩形 -->
            {#if isDragSelecting && (selectRect.width > 0 || selectRect.height > 0)}
                <div
                    class="absolute border-2 border-primary bg-primary/20 pointer-events-none z-10 rounded"
                    style="left: {selectRect.x}px; top: {selectRect.y}px; width: {selectRect.width}px; height: {selectRect.height}px;"
                ></div>
            {/if}
                        ...
                        <div
                            ...
                            onclick={(e) => {
                                ...
                                isSelectionMode
                                    ...
                                    : (sessionStorage.setItem("charactersFromUrl", getCurrentListUrl()), goto(`/characters/${card.id}?from=${encodeURIComponent(getCurrentListUrl())}`))}
                            }
                            onkeydown={(e) => {
            ...
            <div class="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 w-[92%] sm:w-auto">
                <div class="bg-popover/95 backdrop-blur border shadow-xl rounded-2xl sm:rounded-full p-3 sm:px-6 sm:py-3 flex flex-col sm:flex-row items-center gap-3 sm:gap-4 animate-in slide-in-from-bottom duration-300">
                    ...
                    <div class="flex items-center gap-2 flex-wrap sm:flex-nowrap w-full sm:w-auto">
                        ...
                        <Button size="sm" class="flex-1 sm:flex-none" onclick={() => batchTagDialogOpen = true}>
                                 标签
                        </Button>
                        <Button
                            size="sm"
        ...
        {/if}
        <!-- 批量修改标签对话框 -->
<Dialog.Root bind:open={batchTagDialogOpen}>
    <Dialog.Content class="max-w-sm">
        <Dialog.Header>
            <Dialog.Title>批量修改标签</Dialog.Title>
        </Dialog.Header>
        <div class="py-4 space-y-4">
            <p class="text-sm text-muted-foreground">
                将修改选中的 {selectedCardIds.size} 个角色：
            </p>
            <div class="space-y-2">
                <label class="text-sm font-medium">添加标签（逗号分隔）</label>
                <Input
                    placeholder="例如：战斗, 日常"
                    bind:value={batchAddTagInput}
                />
            </div>
            <div class="space-y-2">
                <label class="text-sm font-medium">移除标签（逗号分隔）</label>
                <Input
                    placeholder="例如：旧标签"
                    bind:value={batchRemoveTagInput}
                />
            </div>
        </div>
        <Dialog.Footer>
            <Button variant="ghost" onclick={() => batchTagDialogOpen = false}>
                取消
            </Button>
            <Button onclick={confirmBatchTagUpdate}>
                确认修改
            </Button>
        </Dialog.Footer>
    </Dialog.Content>
